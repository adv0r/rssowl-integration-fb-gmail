...
public class NewsFilterTest {
  ...
  public void testStopFilter() throws Exception {
    ...
    assertEquals(3, news.size());
//    assertEquals(3, bm.getNewsCount(EnumSet.of(INews.State.NEW)));
//    assertEquals(2, bm.getNewsCount(EnumSet.of(INews.State.UNREAD)));
//    for (INews newsitem : news) {
...
//        assertEquals(INews.State.UNREAD, news3.getState());
//    }
  }
...
//    }
  ...
  public void testComplexFilter() throws Exception {
    ...
    assertEquals(0, bm.getNewsCount(EnumSet.of(INews.State.UNREAD)));
...
//    assertEquals(1, bm.getNewsCount(EnumSet.of(INews.State.NEW)));
    ...
    assertEquals(2, bin1.getNewsCount(EnumSet.of(INews.State.UNREAD)));
    ...
    assertEquals(2, bin2.getNewsCount(EnumSet.of(INews.State.UNREAD)));
  ...
  public void testFilterOnDuplicateNews() throws Exception {
    ...
    news1_feed1.setState(INews.State.UNREAD);
    ...
    assertEquals(1, bm2.getNewsCount(EnumSet.of(INews.State.UNREAD)));
    for (INews newsitem : news) {
      assertEquals(1, newsitem.getLabels().size());
      assertEquals(INews.State.UNREAD, newsitem.getState()); //Because the news is duplicate in another feed and read
      assertEquals(label, newsitem.getLabels().iterator().next());
      assertEquals("New Label", newsitem.getLabels().iterator().next().getName());
    }
  }
  ...
  public void testScopeFilter_Matching() throws Exception {
    ...
    assertEquals(0, bm1.getNewsCount(EnumSet.of(INews.State.UNREAD)));
//    assertEquals(0, news.size());
//    for (INews newsitem : news) {
//      assertEquals(INews.State.UNREAD, newsitem.getState());
//    }
