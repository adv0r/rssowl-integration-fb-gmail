...
public class UndoTest {
  ...
  public void testUndoStateChangeAffectEquivalent() throws Exception {
    ...
    UndoStack.getInstance().addOperation(new NewsStateOperation(Collections.singletonList(news1), INews.State.UNREAD, true));

    DynamicDAO.getDAO(INewsDAO.class).setState(Collections.singletonList(news1), INews.State.UNREAD, true, false);
  ...
  public void testUndoRedoExceedsMaxLimit() throws Exception {
    ...
    for (int i = 0; i < 20; i++) {
      UndoStack.getInstance().addOperation(new NewsStateOperation(Collections.singletonList(news), i % 2 == 0 ? INews.State.UNREAD : INews.State.UNREAD, false));
      DynamicDAO.getDAO(INewsDAO.class).setState(Collections.singletonList(news), i % 2 == 0 ? INews.State.UNREAD : INews.State.UNREAD, false, false);
    }
