...
public class CoolBarAdvisor {
  ...
  public enum CoolBarItem {
    ...
    /** Sticky */
    //SPM DELETION  STICKY("org.rssowl.ui.actions.MarkSticky", Messages.CoolBarAdvisor_STICKY, Messages.CoolBarAdvisor_MARK_STICKY, OwlUI.NEWS_PINNED, OwlUI.getImageDescriptor("icons/obj16/news_pinned_disabled.gif"), 6), //$NON-NLS-1$ //$NON-NLS-2$
  ...
  private void registerListeners() {
    ...
    final ISelectionListener selectionListener = new ISelectionListener() {
      public void selectionChanged(IWorkbenchPart part, ISelection selection) {
        //SPM DELETION update(CoolBarItem.MARK_READ, selection, part, false);
        update(CoolBarItem.MOVE, selection, part, false);
        update(CoolBarItem.COPY, selection, part, false);
        //SPM DELETION update(CoolBarItem.STICKY, selection, part, false);
    ...
    final IPartListener partListener = new IPartListener() {
      public void partActivated(IWorkbenchPart part) {
        ...
        //SPM DELETION update(CoolBarItem.MARK_ALL_READ, null, part, false);
      ...
      public void partClosed(IWorkbenchPart part) {
        ...
        if (OwlUI.getEditorReferences().isEmpty()) {
          ...
          //SPM DELETION update(CoolBarItem.MARK_ALL_READ, null, null, false);
        }
  ...
  private void run(Action wrappingAction, CoolBarItem item, ToolBarManager manager) {
    switch (item) {
      ...
      case STICKY: {
        IStructuredSelection selection = OwlUI.getActiveFeedViewSelection();
        if (selection != null && !selection.isEmpty())
          //SPM DELETION new MakeNewsStickyAction(selection).run();
          break;
      }/*
  ...
  private void update(IAction action, CoolBarItem item, ISelection selection, IWorkbenchPart part) {
    switch (item) {
      ...
      case PRINT:
        ...
        /* Update Mark All Read //SPM DELETION
      case MARK_ALL_READ:
        action.setEnabled(part instanceof FeedView || OwlUI.getActiveFeedView() != null);
        break;
         */
