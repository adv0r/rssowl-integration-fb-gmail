  ...
  private void processEntry(Element element, IFeed feed) {
    ...
    for (Iterator< ? > iter = newsChilds.iterator(); iter.hasNext();) {
      ...
      /* Content / Summary */
        ...
        {
          news.setDescription(getContent(child) +FacebookLikeUtil.getButtonCode(itemUrl));
          processNamespaceAttributes(child, news);
        }
        else {
      ...
      /* Link */
        ...
        if (rel == null || "alternate".equals(rel)) {//$NON-NLS-1$
          ...
          if (uri != null){
            news.setLink(uri);
            itemUrl = child.getAttributeValue("href"); //$NON-NLS-1$
          }
          processNamespaceAttributes(child, news);
        }
        ...
        /* Enclosure */
          ...
          if (uri != null){
            ...
            if(! Owl.getPreferenceService().getGlobalScope().getBoolean(DefaultPreferences.HIDE_MP3)){
              if (Mp3Util.isMp3(child.getAttributeValue("type"))){ //$NON-NLS-1$
